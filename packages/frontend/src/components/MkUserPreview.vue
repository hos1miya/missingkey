	...
	<div v-if="showing" class="fxxzrfni _popup _shadow" :style="{ zIndex, top: top + 'px', left: left + 'px' }" @mouseover="() => { emit('mouseover'); }" @mouseleave="() => { emit('mouseleave'); }">
		<div v-if="user != null" class="info">
			...
			<div class="description">
				<Mfm v-if="user.description" :text="user.description" :author="user" :i="$i ?? undefined"/>
			</div>
			<div class="status">
				<div>
...
let user = $ref<pleaides.entities.UserDetailed | null>(null);
let top = $ref(0);
let left = $ref(0);

function showMenu(ev: MouseEvent) {
	os.popupMenu(getUserMenu(user), ev.currentTarget ?? ev.target ?? undefined);
}
